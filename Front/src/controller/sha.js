function sha256ByDaniel(word) {
  ...
  let palavra = palavraOriginal.split("");
  let binarioArr = [];
  ...
  function transformaEmBinario(palavra) {
    ...
    for (var i = 0; i < palavra.length; i++) {
      ...
      let valorBinario = ("00000000" + palavra[i].charCodeAt(0).toString(2))
  ...
  function adicionarTamanhoDaMensagem(binarioArr, tamanhoMensagem) {
    ...
    for (var i = 0; i < 64 - comprimento; i++) {
      tamanhoMensagemBinario = "0" + tamanhoMensagemBinario;
    }


    let mensagemBinarioArr = []
    ...
    for (var i = 0; i < mensagemBinarioArr.length; i++) {
  ...
  function modificaValores(w) {
    ...
    let novoW = w;
    for (var i = 16, j = 0; i < w.length; i++, j++) {
      let s0, s1;
